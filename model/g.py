...
class G_for_CAM(nn.Module):
    def __init__(self):
        super(G_for_CAM, self).__init__()
        
        # get the pretrained VGG19 network
        self.model = googlenet(pretrained=True)
        
        # disect the network to access its last convolutional layer
        self.features_conv = nn.Sequential(*list(self.model.children())[:-6])
        
        # get the max pool of the features stem
        self.avg_pool = nn.Sequential(*list(self.model.children())[-6:-2])
